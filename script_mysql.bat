...
echo     data[tableName] = rows; >>"%SERVICE_FILE%"
echo   }>>"%SERVICE_FILE%"
...
echo   return rows[0] ^|^| null; >>"%SERVICE_FILE%"
echo }>>"%SERVICE_FILE%"
...
echo   if (keys.length === 0) return null;>>"%SERVICE_FILE%"
echo   const setClause = keys.map(key =^> `\`^${key}\` = ?`).join(', ');>>"%SERVICE_FILE%"
echo   const sql = `UPDATE \`${table}\` SET ${setClause} WHERE id = ?`; >>"%SERVICE_FILE%"
echo   const [result] = await conn.query(sql, [...values, id]);  >>"%SERVICE_FILE%"            
